...
<script>
	// This script represents the logic and functionality for the ProfilePage component.
	...
	export default {
		...
		methods: 
		{
			async fetchUser(id) 
			{
				if (id === 0) 
				{
					await axios.get(`${process.env.VUE_APP_HOST_URL}:3000/Profilemyuser`, {withCredentials: true})
					.then((response) => 
					{
				...
				{
					await axios.get(`${process.env.VUE_APP_HOST_URL}:3000/Profileid/${id}`, {withCredentials: true})
					.then((response) => 
					{
						this.user = response.data
					})
